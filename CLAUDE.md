...
- **Bind-mode `/work` writeback is uid-remapped, no chown needed.** Verified on the Linux-VM virtiofs daemon (Ubuntu 24.04, overlayfs — i.e. a Colima/Lima-style daemon, not Docker Desktop macOS): a host file owned by `501:20` shows as the session uid inside the container (e.g. `1000:1000` for agent), `agent`/1000 writes to `/work` succeed with no permission denial, and the file lands back on the host owned by `501:20` (the host user). So edits made in-container in bind mode write straight through to the live host tree with correct host ownership — no `chown` dance. This **diverges from** the `[[mounts]]` chown note above (which says VirtioFS preserves host uid so agent/1000 can't write); that note holds for the Docker Desktop macOS file-sharing path and for mounts under `/home/agent` — virtiofs uid behavior is daemon-dependent, so keep `chown = true` available for the cases that need it.
